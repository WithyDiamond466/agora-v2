...
- The final implementation, `2a83763`, passed GitHub Actions on Windows, macOS, and Linux: dependency installation, 337 offline tests, render and JavaScript checks, the mock workflow, and first-use launchers. [Passing CI run](https://github.com/WithyDiamond466/agora-v2/actions/runs/34042486413).
- Fresh Python 3.12 environment on Linux installed the constrained dependencies and passed `pip check`.
- The offline suite passed 337 tests, with 4 optional local-model tests excluded. The final public code also passed all 337 tests in a fresh GitHub clone with its own virtual environment.

...
- The shipped `Alex_Example.pdf` also produced a stored real-model result, 22/35 against the demonstration course rubric. This was an integration test, not an instructor-calibrated grade.
...
- Course chat did not pass. Gemma printed a code block resembling a tool call, but the saved record contained zero executed course tools. The smoke script now requires a successful tool execution, as well as the expected provider and a nonempty answer.
